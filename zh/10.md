...
An implicit assumption in everyth ing we’ve done so far is that a single program ismodifying our data structures. In Java, one can have the eﬀect of multiple programsmodifying an object, due to the existence of threads
...
- Accesses from separate th reads that make any reference to modiﬁable state are mutually excluded; only one thread at a time holds a lock on a `SmallMailbox` object.
...
​	In the message-passing world, threads are independent sequential programs than send each other messages. They read and write messages using methods that correspond to `read` on Java `Readers`, or `print` on Java `PrintStreams`. As a result, one th read is aﬀected by another only when it bothers to “read its messages.”

​	We can get th e eﬀect of message passing by writing our threads to perform all interaction with each other by means of mailboxes. That is, the threads share some set of mailboxes, but share no other modiﬁable objects or variables (unmodiﬁable objects, like Strings, are ﬁne to share)
